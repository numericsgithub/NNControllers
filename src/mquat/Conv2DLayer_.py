...
class Conv2DLayer(ComplexLayer):
    ...
    Parameters: 
        ...
        strides (tuple of 2 ints):
            (b, a) offset of each convolution operation.
        padding (string):
    ...
    def build(self, input_shape):
        ...
        """
        my_input_shape = input_shape.as_list()
        my_input_shape[0] = 1
        self.input_sample = tf.Variable(tf.zeros(my_input_shape, dtype=self.dtype),
                                        name=self.name+"_input_sample", shape=input_shape,
    ...
    def calibrateLayer(self, inputs):
        #self.input_sample.assign(tf.reshape(inputs, [-1]))
        self.input_sample.assign(inputs)
        all_std_filters = tf.range(3.0, 10, 0.2, dtype=tf.float32)
        all_f_errors = tf.map_fn(self.calibrateLayer_f,
                                                            [all_std_filters],
                                                            fn_output_signature=tf.float32,
                                                            parallel_iterations=1)
        #all_errors_ez_vs_lns = all_clipping_error + all_rounding_errors
        selected = tf.argmin(all_f_errors)
        self.f.quant_out.reset()
        self.f.quant_out.std_keep_factor.assign(all_std_filters[selected])
        return 0.0

    def calibrateLayer_f(self, std_filter):
        org_inputs = self.input_sample
        #org_inputs = tf.reshape(org_inputs, self.my_input_shape)
        self.f.quant_out.reset()
        self.f.quant_out.std_keep_factor.assign(tf.reshape(std_filter, ()))
        quant_forward = self.forward_pass(org_inputs, tf.constant(True))
        float_forward = self.forward_pass_fullprecision(org_inputs, tf.constant(True))
        return tf.reduce_sum(tf.abs(quant_forward - float_forward))
    ...
    def call(self, inputs, training):
        ...
        tf.cond(tf.equal(self.inference_counter, 1, name="should_set_bits"),
                lambda: self.calibrateLayer(inputs),
                lambda: 0.0)
    ...
    def forward_pass_fullprecision(self, inputs, training):
        tmp = inputs
        tmp = self.conv2d([tmp, self.f.var])
        if self.b != None:
            tmp = self.bias_add([tmp, self.b.var])
        if self.batch_norm != None:
            tmp = self.batch_norm(tmp, training)
        tmp = self.activation(tmp)
        return tmp
