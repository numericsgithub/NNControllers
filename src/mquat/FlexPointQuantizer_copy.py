...
class FlexPointQuantizer(Quantizer):
    ...
    def __init__(self, name, total_bits, std_keep_factor=0.0, flex_inferences=1, leak_clip=0.0, dtype=DEFAULT_DATATYPE, debug=False, extra_flex_shift=0, set_b_int=None, symmetric=False):
        ...
        # self.test_summary_writer = tf.summary.create_file_writer('logs/fit/'+self.name)
    ...
    def setBitsBeforeAndAfterAuto(self):
        # tf.print(self.name, "USING AUTO STD")
        # tf.print(self.name, "USING AUTO STD")
        # tf.print(self.name, "USING AUTO STD")
        all_std_filters = tf.range(3.0, 8, 0.25, dtype=tf.float32)
        tf.print("FlexPointQuant:", self.name, "Starting to calculate bits before and after with automatic std")
    ...
    def setBitsBeforeAndAfter(self):
        tf.print("FlexPointQuant:", self.name, "Starting to calculate bits before and after")
        # tf.print(self.name, "NOT USING AUTO STD", self.std_keep_factor)
        # tf.print(self.name, "NOT USING AUTO STD", self.std_keep_factor)
        # tf.print(self.name, "NOT USING AUTO STD", self.std_keep_factor)
        inputs = self.input_sample
        inputs = tf.reshape(inputs, [-1])
        ...
        # if self.debug:
        #     tf.print("FlexPointQuant:", self.name, "all samples unique inputs",inputs)
        clipped_inputs = tf.cond(self.std_keep_factor != 0.0, lambda: self.__filterInputs(inputs, self.std_keep_factor), lambda: inputs)
        # Logging.Log(self.name + "_inputs", inputs)
        # Logging.Log(self.name + "_clipped_inputs", clipped_inputs)
    ...
    def quant(self, inputs):
        ...
        def _quant(inputs): # how to remove those parameters without errors?
            ...
            tf.cond(self.inference_counter == self.flex_inferences,
                    lambda: tf.cond(self.std_keep_factor == -1.0, lambda: self.setBitsBeforeAndAfter(), lambda: self.setBitsBeforeAndAfterAuto()),
                    lambda: 0)
            if self.debug:
                tf.cond(self.inference_counter < self.flex_inferences,
            ...
            # tf.print("LogPointQuant:", self.name, "abs error is",
            #          tf.reduce_sum(tf.abs(tmp - inputs)) / tf.cast(tf.size(inputs), tf.float32),
